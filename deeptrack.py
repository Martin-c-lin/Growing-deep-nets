...
def generate_trajectory_image(image_parameters):
    ...
    for particle_radius, particle_bessel_orders, particle_intensities, ellipsoidal_orientation in zip(particle_radius_list, particle_bessel_orders_list, particle_intensities_list, ellipsoidal_orientation_list):
...
# def generate_trajectory_image(image_parameters): # may not work anymore
#     """Generate image with particles.
#
#     Input:
#     image_parameters: list with the values of the image parameters in a dictionary:
...
def create_growing_conv_network(input_shape = (51, 51, 1),conv_base_dim=16):
    ...
    network.summary()
    return network
def fbf_layer_first(max_number_of_features,image_generator,input_shape=(51,51,1),
    sample_sizes = (32, 512), #arbitrarily chosen
    iteration_numbers = (301, 101),
    verbose=0.01,
    feature_step=1
    ):

    """
    Function for creating and training first fbf layer
    Inputs: feature_step-number of features to add at each step
    Outputs: FBF model
    """
    ...
    conv_list=[]
    for i in range(round(max_number_of_features/feature_step)):
        # Create and add new conv feature
        conv_feature = layers.Conv2D(feature_step, (3,3),activation = 'relu',input_shape=input_shape )(input_tensor)
        conv_list.append(conv_feature)
        if(i==0):
            conv_features=conv_feature
        else:
            conv_features = layers.concatenate(conv_list)#layers.concatenate([conv_features,conv_feature])

        intermediate_conv = layers.MaxPooling2D((2,2))(conv_features) # needed later
        intermediate_layer = layers.Flatten()(intermediate_conv)

        output_tensor2 = layers.Dense(3, name='output')(intermediate_layer)

        #Put model together
        model = Model(input_tensor,output_tensor2)
        model.compile(optimizer='rmsprop', loss='mse', metrics=['mse', 'mae'])
        print(model.summary())

        # Train the model
        train_deep_learning_network(model,
                                image_generator,
                                sample_sizes,
                                iteration_numbers,
                                verbose)
...
def create_growing_dense_network(
    input_shape = (51, 51, 1),
    conv_layers_dimensions = (16, 32, 64, 128)):


    """Creates and compiles a deep learning network convolutional base
    ...
    return network
def add_conv_layer(old_network,new_layer_output_size=32):
    from keras import models, layers
    ...
    new_network.compile(optimizer='rmsprop', loss='mse', metrics=['mse', 'mae'])
    return new_network
...
def add_dense_layer(old_network,new_layer_size=32):
    ...
    for i in range(nbr_of_layers-1):
        old_network.layers[i].trainable=False
        new_network.add(old_network.layers[i])
    dense_layer_name = 'dense_' + str(len(new_network.layers)) # Temporary solution

    new_network.add(layers.Dense(new_layer_size,activation='relu',name=dense_layer_name))
    # OUTPUT LAYER
    number_of_outputs = 3
    output_layer = layers.Dense(number_of_outputs,name="output") #name='output'
    new_network.add(output_layer)
    new_network.compile(optimizer='rmsprop', loss='mse', metrics=['mse', 'mae'])
    return new_network

def train_growing_network(
    network,
    image_generator,
    sample_sizes = (32, 128, 512, 2048),
    iteration_numbers = (301, 201, 101, 101),
    ...
    ### train first layer
    train_deep_learning_network(network,
                                 image_generator,
                                 sample_sizes,
                                 iteration_numbers,
                                 verbose)
     # Add convolutional layers and trian them one by one
    if(save_networks):
         network.save(path+"LBL"+"conv_layer"+str(0)+"_size_"+".h5")

    for idx in range(len(conv_layers_dimensions)):
        conv_dim = conv_layers_dimensions[idx]
        print("Adding conv layer of size ",conv_dim)
        network_intermediate = add_conv_layer(network,conv_dim)
        network_intermediate.summary()
        train_deep_learning_network(network_intermediate,
                                    image_generator,
                                    sample_sizes,
                                    iteration_numbers,
                                    verbose)
        del(network)
        network = network_intermediate
        if(save_networks):

            network.save(path+"LBL"+"conv_layer"+str(idx+1)+"_size_"+str(conv_dim)+".h5")
        del(network_intermediate)

    # Add dense layers and train them one by one
    print(" hello")
    network.summary()
    print("hello 2")
    for idx in range(len(dense_layers_dimensions)):
        dense_dim = dense_layers_dimensions[idx]
        network_intermediate = add_dense_layer(network,dense_dim)
        network_intermediate.summary()
        train_deep_learning_network(network_intermediate,
                                    image_generator,
                                    sample_sizes,
        ...
        network = network_intermediate
        if(save_networks):
            network.save(path+"LBL"+"dense_layer"+str(idx)+"_size_"+str(dense_dim)+".h5")
        del(network_intermediate)
...
def get_images_and_targets(image_generator,sample_size,image_shape=(64,64,1)):
    """
...
def track_video(
    ...
    frames = np.zeros((number_frames_to_be_tracked, video_width, video_height))
    ...
    for i in list(frame_to_be_shown):
        for j in list(rows_to_be_shown):
            for k in list(columns_to_be_shown):
                ...
                print ('particle center x = %5.2f px' % predicted_positions_wrt_box[i, j, k, 1])
                print ('particle center y = %5.2f px' % predicted_positions_wrt_box[i, j, k, 0])
...
def credits():
    ...
    return
def get_image_target_set(image_generator,sample_size,image_shape=(51,51,1)):
    """
    Function for generating a set of images and corresponding targets.
    """
    #image_shape = network.get_layer(index=0).get_config()['batch_input_shape'][1:]
    import numpy as np
    ...
    return images,targets
def create_deep_learning_network_no_dense_top(
    input_shape = (51, 51, 1),
    conv_layers_dimensions = (16, 32, 64, 128)):
    """Creates and compiles a deep learning network.
